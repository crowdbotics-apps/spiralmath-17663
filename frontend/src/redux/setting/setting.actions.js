...
const get_settings = () => {
	return (dispatch) => {
		...
		settingService.get_settings().then(
			...
			(error) =>
				dispatch({
					type: settingTypes.GET_SETTINGS_FAILURE,
					payload: error,
				})
		);
	};
...
const createCreator = (data) => {
	return (dispatch) => {
		...
		settingService.createCreator(data).then(
			(data) =>
				dispatch({
					type: settingTypes.CREATE_CREATOR_SUCCESS,
				}),
			(error) =>
				dispatch({
					type: settingTypes.CREATE_CREATOR_FAILURE,
				})
		);
	};
};

const settingStatusChanger = () => {
	return { type: 'SETTING_STATUS_CHANGER' };
};
